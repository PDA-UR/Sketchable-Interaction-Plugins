...
class PenColorDetector:
    ...
    def detect(frame, ids_and_points, DEBUG=False):
        def setup_sliders():
            ...
            # cv2.createTra  ckbar('Pen Thresh', 'sliders', 0, 255, lambda x: PenColorDetector.update_pen_thresh(x))
            ...
            for k, pen_contour in enumerate([r, g, b]):
                ...
                for i, wc in enumerate(tips):
                    ...
                    if pen_contour is not None and len(pen_contour) > 0:
                        ...
                        if min(distances) < distance and min(distances) < 15:
        ...
        target_distance_in_mm = 40

        # computes the distance in px for given mm; computes that distance in px based on 720p, however we know that 3.84 px/mm are present in 4K
        max_distance = 3.84 * target_distance_in_mm / (3840 / 1280)  # 3.84 px per mm in 4K multiplied with 40mm equals 153.6 px for 4cm; 153.6 px / (DEST_WIDTH / SRC_WIDTH)
        ...
        for _id, x, y in ids_and_points:
            ...
            for k, v in pens.items():
                d = distance_point_point(p, v["tip"])

                if d < distance and d < max_distance:
                    color = k
                    distance = d
