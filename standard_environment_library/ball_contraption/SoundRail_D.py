...
class SoundRail_D(SoundEmittable, PhysicalSimulatable, SIEffect):
    regiontype = PySI.EffectType.SI_CUSTOM_NON_DRAWABLE
    regionname = "__ SoundRail D__"
    region_display_name = "SoundRail D"

    def __init__(self, shape: PySI.PointVector = PySI.PointVector(), uuid: str = "", kwargs: dict = {}) -> None:
        kwargs["sound"] = os.getcwd() + "/plugins/study/pde/res/xylophone-d.wav"
        super(SoundRail_D, self).__init__(shape, uuid, "res/xylophone-d.png", SoundRail_D.regiontype, SoundRail_D.regionname, self.configure_body_kwargs(kwargs))
    ...
    def on_play_sound_enter_recv(self):
        ...
        if len(regions) == 1:
            regions[0].register_played_sound("D")
