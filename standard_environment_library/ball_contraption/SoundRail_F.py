...
class SoundRail_F(SoundEmittable, PhysicalSimulatable, SIEffect):
    regiontype = PySI.EffectType.SI_CUSTOM_NON_DRAWABLE
    regionname = "__ SoundRail F__"
    region_display_name = "SoundRail F"

    def __init__(self, shape: PySI.PointVector = PySI.PointVector(), uuid: str = "", kwargs: dict = {}) -> None:
        kwargs["sound"] = os.getcwd() + "/plugins/study/pde/res/xylophone-f.wav"
        super(SoundRail_F, self).__init__(shape, uuid, "res/xylophone-f.png", SoundRail_F.regiontype, SoundRail_F.regionname, self.configure_body_kwargs(kwargs))
    ...
    def on_play_sound_enter_recv(self):
        ...
        if len(regions) == 1:
            regions[0].register_played_sound("F")
